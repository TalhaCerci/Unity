...
public class demir1Gelistir : MonoBehaviour
{
    ...
    void Update()
    {
        //update fonksiyonu dosyada ki herhangi bir değişim anında çalışması için yazılmış fonksiyondur.
        ...
        for (int i = 0; i < Lines.Length; i++)
        {
            ...
            if (bilgiler[0] == "1")
            {
                deger = Convert.ToInt32(bilgiler[10]);
                break;
            }
        }
        GameObject.Find("Demir1Gelistir").GetComponentInChildren<Text>().text = "Bu Seviyeyi Geliştir : " + (deger + 1).ToString();
        //bu fonksiyon içerisinde Demir1'in son seviyesi metin belgesinden okunup 1 fazlası Demir1Gelistir butonunun text'ine yazıldı
    }
    public void gelistir()
    {
        ...
        //içerisinde Demir1'in seviyesini tutan değer seviye değişkenine atandı
        Lines = System.IO.File.ReadAllLines("users.txt");
        for (int i = 0; i < Lines.Length; i++)
        {
            string[] bilgiler = Lines[i].Split(' ');
            if (bilgiler[0] == "1")
            {
                seviye = Convert.ToInt32(bilgiler[10]);
                break;
            }
            satir++;
        }
