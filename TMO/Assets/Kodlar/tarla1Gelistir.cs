...
public class tarla1Gelistir : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < Lines.Length; i++)
        {
            ...
            if (bilgiler[0] == "1")
            {
                deger = Convert.ToInt32(bilgiler[22]);
                break;
            }
        }
        GameObject.Find("Tahil1Gelistir").GetComponentInChildren<Text>().text = "Bu Seviyeyi Geliştir : " + (deger + 1).ToString();
        //bu fonksiyon içerisinde Tarla1'in son seviyesi metin belgesinden okunup 1 fazlası Tahil1Gelistir butonunun text'ine yazıldı
    }
    public void gelistir()
    {
        ...
        //içerisinde Tarla1'in seviyesini tutan değer seviye değişkenine atandı
        Lines = System.IO.File.ReadAllLines("users.txt");
        for (int i = 0; i < Lines.Length; i++)
        {
            string[] bilgiler = Lines[i].Split(' ');
            if (bilgiler[0] == "1")
            {
                seviye = Convert.ToInt32(bilgiler[22]);
                break;
            }
            satir++;
        }
